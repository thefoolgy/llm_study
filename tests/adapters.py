...
def run_embedding(
    ...
    embedding_layer = Embedding(vocab_size, d_model, device)
...
def run_swiglu(
    ...
    swiglu.load_state_dict({'w1_weight.weight': w1_weight,
...
def run_scaled_dot_product_attention(
    ...
    """
    scaled_dot_product_attention = ScaledDotProductAttention(d_k=Q.size(-1))
...
def run_multihead_self_attention(
    ...
    multihead_self_attention = CausalMultiHeadSelfAttention(d_model=d_model, 
                                                            num_heads=num_heads, 
                                                            use_rope = False,
...
def run_transformer_block(
    ...
    """
    dtype, device = in_features.dtype, in_features.device
    ...
    transformer_block.attn.q_proj.weight.data.copy_(weights['attn.q_proj.weight'])
    transformer_block.attn.k_proj.weight.data.copy_(weights['attn.k_proj.weight'])
    transformer_block.attn.v_proj.weight.data.copy_(weights['attn.v_proj.weight'])
    transformer_block.attn.o_proj.weight.data.copy_(weights['attn.output_proj.weight'])

    transformer_block.norm1.g.data.copy_(weights["ln1.weight"])
    transformer_block.norm2.g.data.copy_(weights["ln2.weight"])

    transformer_block.ff.w1_weight.weight.data.copy_(weights["ffn.w1.weight"])
    transformer_block.ff.w2_weight.weight.data.copy_(weights["ffn.w2.weight"])
    transformer_block.ff.w3_weight.weight.data.copy_(weights["ffn.w3.weight"])
...
def run_transformer_lm(
    ...
    with torch.no_grad():
        _copy_param(model.token_emb.weight,
        ...
        for layer_idx in range(num_layers):
            ...
            _copy_param(block.norm1.g, weights[pfx+"ln1.weight"])
            _copy_param(block.norm2.g, weights[pfx+"ln2.weight"])
            _copy_param(block.ff.w1_weight.weight, weights[pfx+"ffn.w1.weight"])
            _copy_param(block.ff.w2_weight.weight, weights[pfx+"ffn.w2.weight"])
            _copy_param(block.ff.w3_weight.weight, weights[pfx+"ffn.w3.weight"])
        _copy_param(model.ln_final.g, weights["ln_final.weight"])
...
def run_rmsnorm(
    ...
    """
    device, dtype = in_features.device, in_features.dtype
    rmsnorm_layer = RMSNorm(d_model, eps, device, dtype)
